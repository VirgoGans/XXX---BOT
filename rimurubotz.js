...
const { getBuffer, fetchJson, fetchText, getRandom, getGroupAdmins, runtime, getName, sendContact, decodeJid, sleep, makeid } = require("./lib/myfunc");
...
const fpayment = {
				...
				message: {
					requestPaymentMessage: {
						currencyCodeIso4217: 'AED',
						amount1000: '50000',
						requestFrom: '0@s.whatsapp.net',
						noteMessage: {
							extendedTextMessage: {
								text: 'ðŸ‘‹ Hay Kak :> ' + pushname
							}
						},
						expiryTimestamp: '50000',
						amount: {
							value: '50000',
							offset: '50000',
							currencyCode: 'AED'
...
const fakestatus = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(nay.chat ? { remoteJid: "status@broadcast" } : {})},message: { "imageMessage": {"url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc","mimetype": "image/jpeg","caption": '©ＸｘＸ － Ｔｅａｍｓ',"fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=","fileLength": "28777","height": 1080,"width": 1079,"mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=","fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=","directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69","mediaKeyTimestamp": "1610993486","jpegThumbnail": thumb,"scansSidecar": "1W0XhfaAcDwc7xh1R8lca6Qg/1bB4naFCSngM2LKO2NoP5RI7K+zLw=="}}}
...
const decodeJid = (jid) => {
    ...
    const getName = (jid, withoutContact  = false) => {
       var id = decodeJid(jid)
        withoutContact = rimurubotz.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = rimurubotz.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? {
            id,
            name: 'WhatsApp'
        } : id === decodeJid(rimurubotz.user.id) ?
            rimurubotz.user :
            (store.contacts[id] || {})
            return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid).getNumber('international')
    }
    
    
    rimurubotz.ev.on('contacts.update', update => {
...
return rimurubotz.relayMessage (from, toduh.message, {messageId: toduh.key.id})
		...
		rimurubotz.sendPresenceUpdate('available', from)
...
fileLength: '999999',
pageCount: '999',
caption : help(prefix, reply, cekUser, namabot, sender),
footer : `© XxX - Team`,
...
mimetype: 'application/pdf',
fileLength: '999999',
pageCount: '999', thumb, caption: `[ *NEW INFO* ] 
• *User* : ${sender.split("@")[0]}
• *Star* : ⭐[1]
• *Hit* : 0
• *Premium* : false
...
if (cekUser("id", sender) == null) return Notdaftar()
if (!isGroup){
if (nay.messages[0].isQuotedMsg == false) return reply("tag Pesan")
if (nay.messages[0].quotedMsg.sender !== botNumber) return reply("Hanya bisa menghapus pesan bot, ... Silahkan reply pesan bot")
await rimurubotz.sendMessage(from, { delete: nay.messages[0].quotedMsg })
...
if (!isBotGroupAdmins){
if (nay.messages[0].quotedMsg.sender !== botNumber) return only("isBotGroupAdmins", rimurubotz, from)
await rimurubotz.sendMessage(from, { delete: nay.messages[0].quotedMsg })
only("sukses", rimurubotz, from)}}
break
case 'tobugstik':
const bugstik = {
		...
		"message": {
			"orderMessage": {
				...
				"status": "INQUIRY",
				"surface": "CATALOG",
				"message": '👋  Kak :> ' + pushname,
				"orderTitle": '👋 Kak :> ' + pushname,
...
let a = []
let b = text.split('|')
if (!b[1]) return rpy(from, 'Format\n' + command + ' halo |ya|gak')
if (b[12]) return rpy(from, 'Kebanyakan pilihan, Format\n' + command + ' halo |ya|gak')
for (let c = 1; c < b.length; c++) {
a.push([b[c]])
			}
let cap = `${text.split('|')[0]}`
sendPoll(from, b, a, nay)
break
case 'bc':
case 'broadcast':
case 'bcall':
if (isOwner) return rpy(from, 'only owner')
...
break
case 'test':
sendKatalog(from, 'Satganz Devs', help(prefix, reply, cekUser, namabot, sender), thumb)
break
case 'add': 
if (cekUser("id", sender) == null) return Notdaftar()
if (!isGroup) return only("isGroup", rimurubotz, from)
...
case 'script': 
if (cekUser("id", sender) == null) return Notdaftar()
rimurubotz.sendMessage(from, { text: 'Ga ada!', externalAdReply: { showAdAttribution: true,
mediaType:  1,
mediaUrl: 'https://wa.me/6281316701742',
title: '「 ❔ 」',
body: sender,
sourceUrl: 'https://wa.me/6281316701742', thumbnail: fs.readFileSync('./media/menfess.jpg')
  }
 }, { quoted: nay1 })
break
...
if (budy.includes("herokuapp")) { reply("Huh?") } 
	...
	if (isMenfess && isTo) {
          ...
          if (isMenfess && isSend) {
			...
			// Anti Spam
			// anti delete
rimurubotz.ev.on('message-delete', async (nay) => {
...
if (m.mtype == 'viewOnceMessage') {
        ...
        if (/video/.test(type)) {
            rimurubotz.sendMessage(from, { video: buffer, caption: msg[type].caption, contextInfo: { externalAdReply :{ mediaUrl: `https://chat.whatsapp.com/DXzNLv2I7mh01ikTbyFXBq`, mediaType: 1, description: 'Join Group Official', title: 'ＸｘＸ － Ｔｅａｍｓ Official', body: footer, thumbnail: thumb, renderLargerThumbnail: 1, sourceUrl: `https://chat.whatsapp.com/DXzNLv2I7mh01ikTbyFXBq`}}}, { quoted : nay })
            rpy(from, '[View Once Video] Detected')
            
        } else if (/image/.test(type)) {
            rimurubotz.sendMessage(from, { image: buffer, caption: msg[type].caption, contextInfo: { externalAdReply :{ mediaUrl: `https://chat.whatsapp.com/DXzNLv2I7mh01ikTbyFXBq`, mediaType: 1, description: 'Join Group Official', title: 'ＸｘＸ － Ｔｅａｍｓ Official', body: footer, thumbnail: thumb, renderLargerThumbnail: 1, sourceUrl: `https://chat.whatsapp.com/DXzNLv2I7mh01ikTbyFXBq`}}}, { quoted : nay })
...
let feature = ["tobugstik","donasi","sticker","owner","afk","toimg","script","confes","report","antilink","welcome","revoke","linkgrup","locked","unlocked","opengroup","closegroup","setdeks","setsubject","demote","promote","add","kick","kicktime","delete","updatestatus","setnamabot","getpp","block","unblock","creategroup","plusstar","minusstar","dellimage","dellvideo","dellsticker","dellaudio","technology","cuttext","neonlight","thundertext","transformer","sketchtext","lighttext","giraffetext","glasstext","signtext","juicetext","typography","potterytext","comictext","ruststyle","steeltext","metalgold","metalgalaxy","rosegold","metalonline","logoonline","stonetext","styletiktok","vintage","graffititext","texteffect","layeredtext","screentext","summertext","baperin1","baperin2","baperin3","baperin4","baperin5","baperin6","baperin7","baperin8","baperin9","baperin10","wangy","sherk","simp","nenen","smeme1","smeme2","smeme3","playmp3","playmp4","ytvideo","ytmp3","tiktokaudio","tiktokvideo","addimage","addvideo","addsticker","addaudio","getimage","getvideo","getsticker","getaudio","listimage","listvideo","liststicker","listaudio","pinterest","wallpaper","igstalk","artinama","cuaca","kusonime","carigrup","happymod","imagesketch","shit","burn","blur","greyscale","pixelate","removebg","beautiful","trash","jail","wanted","rip","gay","invert","imagesketchme","shitme","burnme","blurme","greyscaleme","pixelateme","removebgme","beautifulme","trashme","jailme","wantedme","ripme","gayme","invertme","imagesketchtag","shittag","burntag","blurtag","greyscaletag","pixelatetag","removebgtag","beautifultag","trashtag","jailtag","wantedtag","riptag","gaytag","inverttag","ttpwhite","ttpyellow","ttpblue","ttpred","ttpgreen","ttpblack","ttpbrown","ttpteal","ttpsilver","ttppurple","ttpgray","ttporange","ttpmaroon","ttpaquamarine","ttpcoral","ttpfuchsia","ttpwheat","ttplime","ttpcrimson","ttpkhaki","ttpmagenta","ttpplum","ttpolive","ttpcyan"]
    if (isCommand) {
		let help = Object.values(feature).filter(v => v.feature && !v.disabled).map(v => v.feature).flat(1)
	if (help.includes(command)) {
		let mean = didyoumean(command, help)
		let buttonMessage = { document: fs.readFileSync('./menu.doc'), jpegThumbnail: thumb, fileName: 'DID YOU MEAN', mimetype: 'application/pdf', fileLength: '999999', pageCount: '999', caption : `👋 Hai ${pushname} @${sender.split("@")[0]}, Apakah yang kamu maksud: *${prefix + mean}*\nAbaikan jika salah`, footer : `© XxX - Team`, buttons: [{ buttonId: '.' + mean + text, buttonText: { displayText: 'I Y A' }, type: 1 }, { buttonId: '', buttonText: { displayText: 'B U K A N' }, type: 1 }], headerType: 2, contextInfo: { externalAdReply : { mediaUrl: `https://chat.whatsapp.com/DXzNLv2I7mh01ikTbyFXBq`, mediaType: 1, description: 'Join Group Official', title: 'ＸｘＸ － Ｔｅａｍｓ Official', body: footer, thumbnail: thumb, renderLargerThumbnail: 1, sourceUrl: `https://chat.whatsapp.com/DXzNLv2I7mh01ikTbyFXBq`}}}
rimurubotz.sendMessage(from, buttonMessage)
}
}
